...
// ─── GLOBALS ──────────────────────────────────────────────────────────────────
const unsigned long INTERVAL = 30000UL - 400UL;  // ~30 seconds
...
const char* mqtt_topic  = "/Set1";

// ─── OLED ─────────────────────────────────────────────────────────────────────
U8G2_SH1106_128X64_NONAME_F_HW_I2C u8g2(
  U8G2_R0, U8X8_PIN_NONE, 10, 8);

// ─── SENSORS ──────────────────────────────────────────────────────────────────
const int sensorPins[3] = { 4, 0, 3 };
...
// ─── PUBLISH ──────────────────────────────────────────────────────────────────
void publishSensorData(int v[3], float tempC) {
  StaticJsonDocument<200> doc;
  doc["C1"] = v[0];
  doc["C2"] = v[1];
  doc["T1"] = serialized(String(tempC, 1));
  doc["R3"] = v[2];
...
void loop() {
  ...
  // ── Read temperature ──
  float tempC = selected ? ds.getTempC() : -127.0;
  ...
  Serial.printf("C1: %d  C2: %d  T: %.1f C  R3: %d  Reading#: %lu\n",
                v[0], v[1], tempC, v[2], readingCount);

  // ── OLED display ──
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x12_tf);

  const char* labels[] = { "C1", "C2", "R3" };
  for (int i = 0; i < 3; i++) {
  ...
  u8g2.setCursor(0, 12 + 3 * 12);
  u8g2.print("T: ");
  if (tempC == -127.0) u8g2.print("ERR");
  else {
    u8g2.print(tempC, 1);
    u8g2.print(" C");
  }
